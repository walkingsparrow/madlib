...
class FistaState
{
  ...
  protected:
    void rebind ()
    {
        ...
        task.xmean.rebind(&mStorage[6 + task.dimension], task.dimension);
        task.ymean.rebind(&mStorage[6 + 2 * task.dimension]);
        task.tk.rebind(&mStorage[7 + 2 * task.dimension]);

        algo.numRows.rebind(&mStorage[8 + 2 * task.dimension]);
        algo.gradient.rebind(&mStorage[9 + 2 * task.dimension], task.dimension);
    }
